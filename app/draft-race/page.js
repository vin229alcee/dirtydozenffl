...
export default function DraftRacePage(){
  ...
  useEffect(()=>{if(!supabase)return;let active=true;(async()=>{
    ...
    const {data:ch}=await supabase.from('commissioner_challenges').select('*').eq('season',SEASON).eq('is_active',true).order('week',{ascending:false}).limit(1).maybeSingle();
    let sched={games:[]};try{sched=await fetch('/api/pick-em',{cache:'no-store'}).then(r=>r.json())}catch{}
    if(!active)return;setTeams(t||[]);setEntries(e||[]);setChallengeEntries(c||[]);setAdjustments(a||[]);setChallenge(ch||null);setSession(s?.session||null);setSchedule(sched||{games:[]});
  })();return()=>{active=false}},[]);
  ...
  async function submit(){if(!session||!managerTeam||!challenge)return setStatus('Sign in with a linked manager account first.');if(!answer.trim())return setStatus('Enter your answer first.');const {error}=await supabase.from('commissioner_challenge_entries').upsert({challenge_id:challenge.id,manager_team_id:Number(managerTeam.team_id),answer:answer.trim(),submitted_at:new Date().toISOString()},{onConflict:'challenge_id,manager_team_id'});setStatus(error?error.message:'Challenge answer submitted.');if(!error)setAnswer('');}
  ...
  return <PageShell title="DRAFT RACE" kicker="EARN YOUR 2027 DRAFT SLOT">
    ...
    <section className="commissionerGrid" style={{marginBottom:18}}>
      ...
      <article className="panel"><div className="panelTitle"><h3>WEEKLY COMMISSIONER CHALLENGE</h3><span>{challenge?`WEEK ${challenge.week}`:'PENDING'}</span></div>{challenge?<><h2 style={{fontFamily:'Oswald'}}>{challenge.title}</h2><p>{challenge.description||challenge.prompt||''}</p><p><strong>{challenge.points} Draft Race pts</strong></p><textarea rows={4} value={answer} onChange={e=>setAnswer(e.target.value)} placeholder="Enter your answer…" style={{width:'100%',margin:'10px 0'}}/><button className="primaryButton" onClick={submit}>Submit Challenge</button></>:<div className="emptyPanel">The commissioner has not posted a challenge yet.</div>}{status&&<p>{status}</p>}</article>
    </section>
