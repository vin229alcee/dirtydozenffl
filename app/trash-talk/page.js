...
export default function TrashTalkPage(){
  ...
  async function loadPosts(){
    ...
    const [{data:teamRows},{data:postRows,error}]=await Promise.all([
      ...
      supabase.from('trash_talk_posts').select('id,author_user_id,team_id,body,image_path,created_at,updated_at').order('created_at',{ascending:false}).limit(100)
    ]);
    if(error){
      if(error.code==='42P01')setStatus('Trash Talk database setup is still pending.');
      setLoading(false);
      return;
    }
    setTeams(teamRows||[]);
  ...
  async function loadIdentity(currentSession){
    ...
    const {data:row,error}=await supabase.from('manager_access').select('team_id,email').ilike('email',userEmail).maybeSingle();
    if(error){if(error.code==='42P01')setStatus('Manager access setup is still pending.');return;}
  ...
  async function submitPost(e){
    ...
    let imagePath=null;
    if(file){
      imagePath=`${session.user.id}/${Date.now()}-${safeFileName(file.name)}`;
      const {error:uploadError}=await supabase.storage.from('trash-talk').upload(imagePath,file,{cacheControl:'3600',upsert:false,contentType:file.type});
      if(uploadError){setStatus(uploadError.message);setPosting(false);return;}
    }
    const {error}=await supabase.from('trash_talk_posts').insert({author_user_id:session.user.id,team_id:Number(access.team_id),body:text||null,image_path:imagePath});
    if(error){
      if(imagePath)await supabase.storage.from('trash-talk').remove([imagePath]);
  ...
  async function deletePost(post){
    if(!supabase||post.author_user_id!==session?.user?.id)return;
    ...
    if(post.image_path)await supabase.storage.from('trash-talk').remove([post.image_path]);
    await loadPosts();
  }

  function imageUrl(path){
    if(!path||!supabase)return'';
    return supabase.storage.from('trash-talk').getPublicUrl(path).data.publicUrl;
  }
  ...
  return <PageShell title="TRASH TALK" kicker="LEAGUE SOCIAL FEED">
    ...
    {!session?<section className="panel trashTalkLogin"><div><span className="eyebrow">MANAGER ACCESS</span><h2>Sign in to post</h2><p>Use the same Supabase login tied to your league manager account.</p></div><form onSubmit={signIn}><input type="email" placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)} required/><input type="password" placeholder="Password" value={password} onChange={e=>setPassword(e.target.value)} required/><button className="primaryButton" type="submit">Sign In</button></form>{status&&<p className="trashTalkStatus">{status}</p>}</section>:
    ...
    <section className="trashTalkFeed" aria-live="polite">
      {loading?<div className="panel emptyPanel">Loading the locker room…</div>:posts.length?posts.map(post=>{const t=teamsById[Number(post.team_id)]||{};const mine=post.author_user_id===session?.user?.id;return <article className="panel trashTalkPost" key={post.id}><div className="trashTalkPostHead"><div><strong>{t.name||'Dirty Dozens'}</strong><span>{t.manager||'League Manager'} · {formatTime(post.created_at)}</span></div>{mine&&<button type="button" onClick={()=>deletePost(post)}>Delete</button>}</div>{post.body&&<p>{post.body}</p>}{post.image_path&&<img src={imageUrl(post.image_path)} alt="Meme posted to Dirty Dozens trash talk" loading="lazy"/>}</article>}):<div className="panel emptyPanel"><h3>No trash talk yet.</h3><p>Somebody has to fire the first shot.</p></div>}
    </section>
  </PageShell>;
}
