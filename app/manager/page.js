...
export default function ManagerDashboard(){
  ...
  const [dirtyAwards,setDirtyAwards]=useState([]);
  ...
  async function loadDashboard(){
    ...
    setIsCommissioner(!!link.is_commissioner);
    const teamId=Number(link.team_id);
    const [teamRes,teamsRes,matchRes,rankRes,champRes,highRes,dirtyRes,trashRes]=await Promise.all([
    ...
    setTeam(teamRes.data||null);setTeams(teamsRes.data||[]);setMatchups(matchRes.data||[]);setRankings(rankRes.data||[]);setChampions(champRes.data||[]);setHighScores(highRes.data||[]);setDirtyAwards(dirtyRes.data||[]);setTrash(trashRes.data||[]);setLoading(false);
  }
  ...
  const currentGames=matchups.filter(m=>Number(m.season)===SEASON);
  const recentGames=currentGames.slice(0,5);
  const career={wins:0,losses:0,pointsFor:0,pointsAgainst:0};
  const rivalries={};
  ...
  if(!session)return <PageShell title="MANAGER HQ" kicker="PERSONAL DASHBOARD"><section className="panel trashTalkLogin"><div><span className="eyebrow">MANAGER LOGIN</span><h2>Your team. Your history. Your receipts.</h2><p>Sign in with the account linked to your Dirty Dozens team.</p></div><form onSubmit={signIn}><input type="email" placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)} required/><input type="password" placeholder="Password" value={password} onChange={e=>setPassword(e.target.value)} required/><button className="primaryButton">Open Manager HQ</button></form>{status&&<p className="trashTalkStatus">{status}</p>}<p style={{marginTop:14}}><Link href="/join">Need an account? Join here →</Link></p></section></PageShell>;
  ...
  return <PageShell title="MANAGER HQ" kicker="PERSONAL DASHBOARD">
    ...
    <section className="commissionerGrid">
      <div className="panel"><div className="panelTitle"><h3>RECENT RESULTS</h3><span>2026</span></div>{recentGames.length?recentGames.map(m=>{const home=Number(m.team1_id)===Number(team.id),opp=teamById[home?Number(m.team2_id):Number(m.team1_id)],mine=home?m.team1_score:m.team2_score,theirs=home?m.team2_score:m.team1_score,win=Number(mine)>Number(theirs);return <div className="recordRow" key={m.id}><span>WEEK {m.week} · {win?'W':'L'} vs {opp?.name||'Opponent'}</span><strong>{score(mine)}–{score(theirs)}</strong></div>}):<div className="emptyPanel">No completed results yet.</div>}</div>
    ...
    <section className="commissionerGrid" style={{marginTop:18}}>
      <div className="panel"><div className="panelTitle"><h3>TOP RIVALRY</h3><Link href="/rivalries">FULL RIVALRIES</Link></div>{topRivalry?<><h2 style={{fontFamily:'Oswald',margin:'0 0 8px'}}>{teamById[topRivalry.oppId]?.name||'Opponent'}</h2><p>{topRivalry.games} meetings · {topRivalry.wins}-{topRivalry.losses} record</p><div className="recordRow"><span>Points scored</span><strong>{score(topRivalry.pf)}</strong></div><div className="recordRow"><span>Points allowed</span><strong>{score(topRivalry.pa)}</strong></div></>:<div className="emptyPanel">Rivalry data will grow as matchup history is added.</div>}</div>
      <div className="panel"><div className="panelTitle"><h3>RECENT TRASH TALK</h3><Link href="/trash-talk">OPEN FEED</Link></div>{trash.length?trash.map(p=><article key={p.id} style={{padding:'10px 0',borderBottom:'1px solid #2b3945'}}><small style={{color:'#9aa6b2'}}>{formatDate(p.created_at)}</small><p style={{margin:'5px 0'}}>{p.body||'Meme post'}</p></article>):<div className="emptyPanel">No posts yet. Go start something.</div>}</div>
    </section>

    <section className="panel" style={{marginTop:18}}><div className="panelTitle"><h3>ACHIEVEMENTS</h3><span>FRANCHISE RESUME</span></div><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:12}}><div><strong style={{fontFamily:'Oswald',fontSize:24}}>{champions.length}</strong><p>League Championships</p></div><div><strong style={{fontFamily:'Oswald',fontSize:24}}>{highScores.length}</strong><p>Weekly High Scores</p></div><div><strong style={{fontFamily:'Oswald',fontSize:24}}>{dirtyAwards.length}</strong><p>Dirty Player Selections</p></div><div><strong style={{fontFamily:'Oswald',fontSize:24}}>{trash.length}</strong><p>Recent Trash Talk Posts</p></div></div></section>
  </PageShell>;
}
